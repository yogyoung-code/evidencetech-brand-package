...
- **字重**：Medium (500)（与 logo 设计稿一致）
- **字符样式**：**Mixed case**（保留驼峰拼写，**不**应用 `text-transform: uppercase`）
- **字距**：`letter-spacing: -0.025em`（与 logo 设计稿一致，字距略微收紧）
- **行高**：`line-height: 1.2`（与 logo 设计稿一致）
- **字体导入**：
...
四个产品分两种 wordmark 模式：DE/SE/DI 是 **双色文字模式**，RCP 是 **缩写文字 + 彩色横线模式**。两种模式共用 Teko Medium + mixed-case 基础。
...
```css
.wordmark { font: 500 32px/1.2 "Teko", sans-serif; letter-spacing: -0.025em; white-space: nowrap; }
...
.rcp-logo {
  ...
  font: 500 32px/1.2 "Teko", sans-serif; letter-spacing: -0.025em;
...
- Teko Medium (500) 唯一字重；mixed-case；letter-spacing -0.025em；line-height 1.2
...
```
DEEPEVIDENCE              ← Teko Medium, #2563EB, 32px
...
对外物料（UI、市场、PR）渲染层：
- Wordmark 显示规则（按产品）：
  - DE / SE / DI → mixed-case Teko Medium（`DeepEvidence` 的字面拼写，Teko 字形渲染）
...
::: tip 文档来源
本页由 `scripts/sync-docs.mjs` 自动从 kit 根的 [`02-brand-architecture.md`](https://github.com/) 同步。**不要直接编辑此文件**——所有修改请改源文件后重跑 sync 脚本。
:::
