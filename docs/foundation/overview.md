...
| 文件 | 状态 | 内容 |
|---|---|---|
| [`README.md`](/foundation/overview) | ✅ v1.0 | 本文件，入口与索引 |
| [`00-architecture-locked.md`](/foundation/architecture) | ✅ Locked | 架构层决策：branded house、四产品色彩 / 字体矩阵、信任层、Endorsement signature |
| [`01-gap-audit.md`](/foundation/audit) | ✅ v0.1 | 三个 Cloud Run 原型（DE/SE/RCP）与本 kit 的差距清单 |
| [`02-brand-architecture.md`](/architecture/brand-architecture) | ✅ v0.6 | Logo（图标 + 双色 wordmark）+ Endorsement signature + VGI mark + RCP 贯穿横线 + 命名系统 |
| [`03-brand-voice.md`](/voice/brand-voice) | ✅ v0.1 | CN 主 EN 辅、4 条原则、双受众 messaging matrix、扩展禁用词、写作微规则 |
| [`04-visual-system.md`](/visual/visual-system) | ✅ v0.2 | 三大信任徽章 + Nano-Citation + CoT 动画 + Diff View + Progressive Disclosure + 图像政策 |
| [`05-rectification-checklist.md`](/governance/rectification) | ✅ v0.1 | 三个原型按 kit 整改的具体清单（不做实际改造，只列清单） |
| [`06-governance.md`](/governance/governance) | ✅ v0.1 | 版本号、变更流程、与母品牌 sync cadence |
...
- 完整治理流程见 [`06-governance.md`](/governance/governance)
...
::: tip 文档来源
本页由 `scripts/sync-docs.mjs` 自动从 kit 根的 [`README.md`](https://github.com/) 同步。**不要直接编辑此文件**——所有修改请改源文件后重跑 sync 脚本。
:::
